...
def banner():
	os.system("clear")
	print(f"""___  _  _ _  _ ___     _ ___  
|  \ |  | |\/| |__] __ | |  \ 
|__/ |__| |  | |       | |__/ 
________________________________________________
 [*] Author   : Faizi
 [*] GitHub   : personal
 [*] WhatsApp : +923424251088
 [*] Version  : 0.1
________________________________________________
\t{garis} author by : Faizi""")              
...
def login():
	banner()
	x=f"{P2}Hello user script dump»ID:)\n{P2}please select the cookie login feature to proceed to the multi fast dump menu.. if you don't understand anything, you can type {M2}help {P2}to ask for help !!"
	vprint(panel(x,style=f"{H3}"))
	print("")
	x=f"{P2}[01] login with cookie\n{P2}[02] report bug script\n{P2}[{M2}00{P2}] exit "
	vprint(panel(x,style=f"{H3}"))
	cukuf = input(f" {P}[{H}•{P}] Select : {H}")
	if cukuf in ["help","Help","HELP"]:
		print("")
		x=f"{P2}whatsapp admin *--> {H2}083843614174 {P2}harap chat klo ada kepentingan yang mau disampaikan ke author dmf\nini klo gak bisa diarahin ke whastapp admin yakk"
		vprint(panel(x,style=f"{H3}"))
		print("")
		x=f"{P2}sedang diarahkan ke whastapp author"
		vprint(panel(x,style=f"{H3}"))
		os.system('xdg-open https://wa.me/6283843614174?text=bang+cara+pake+script+abang+kek+mana?')
...
def login_cookie():
	print("")
	#testi_ua()
	x = f"{P2}don't use personal accounts!! you have to use a victim account to take cookies"
	vprint(panel(x,style=f"{H3}"))
	cookie = str(input(f"{garis} enter cookies :"+H+" "))
	with requests.Session() as xyz:
		try:
			jalan(f"{garis} in progress please wait ")
			...
			x=f"{P2}{token}"
			vprint(panel(x,style=f"{H3}"))
			input(f"{garis} enter to go to the menu ")
			menu()
		except requests.exceptions.ConnectionError:
			x=f"{P2}problem internet connection"
			vprint(panel(x,style=f"{H3}"))
...
def menu():
	...
	try:lok = x["locale"]
		...
		lok = M+"-"+P
	print("")
	x=f"{P2}this is not fb crack script!!.. this is just a dump of the id to make it simple..\later next crack fb from the dump file!!"
	vprint(panel(x,style=f"{H3}"))
	print("")
	x=f"{P2} ┌─ Welcome {K2}{nama}\n{P2} │ └─ your date of birth : {H2}{pko}\n{P2} │ └─ your ID : {H2}{tumbal_id}\n{P2} │ └─ your Ip : {H2}{IP}\n{P2} │ ┌─ your country : {H2}{nibba}\n{P2} └─ current date : {H2}{sekarang}"
	vprint(panel(x,style=f"{H3}"))
	print("")
	x=f"{P2}[01] Public ID Dump\n{P2}[02] Dump ID Followers\n{P2}[03] Dump ID Post likes\n{P2}[04] Multi Public ID Dump\n{P2}[05] Dump Multi ID Followers \n{P2}[06] check target account info\n{P2}[07] respond to this script\n{P2}[08] report bug script\n{P2}[09] tracked ip\n{P2}[{M2}00{P2}] exit/happy cookie"
	vprint(panel(x,style=f"{H3}"))
	hikmat = input(f"{garis} Select : {H}")
	if hikmat in ["1","01"]:
		dump_public()
	elif hikmat in ["2","02"]:
		dump_followers()
...
def pilpil():
	hikmatz = input(f"{garis} Select : {H}")
	if hikmatz in ["1","01"]:
		tracked_sendiri()
...
def dump_publicv2():
	...
	print("")
	x=f"{P2}copy the results match so you don't repeat the dump again!!"
	vprint(panel(x,style=f"{H3}"))
	input(f"{garis} enter to return")
	menu()
